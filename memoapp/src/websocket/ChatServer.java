...
// ServerEndpoint 어노테이션으로 웹소켓 서버를 지정한다.
// ws://호스트:포트번호/CahttingServer
@ServerEndpoint("/ChattingServer")
public class ChatServer {
	// 새로 접속한 클라이언트의 세션을 지정하는 컬렉션을 생성한다.
	//Collections클래스의  synchronizedSet() 메서드는 안전하게 Set컬렉션을 생성해준다.
	//여러 클라이언트가 동시에 접속해도 문제가 발생하지 않게 해준다.
	private static Set<Session> clients = Collections.synchronizedSet(new HashSet<>());
	
	// 클라이언트가 접속했을때 실행할 매서드를 정의한다. 이 메서드에서는 단순하게 clients 컬렉션에 세션을 추가한다.
	...
	//클라이언트로부터 메세지를 받았을때 실행되는 메서드를 정의한다. 클라이언트가 보낸 메시지와 
	...
	//클라이언트가 접속을 종료했을때 실행되는 메서드를 정의한다. clients에서 해당 세션을 삭제한다.

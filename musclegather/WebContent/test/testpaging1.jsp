...
<body>
	<div class="notify">
		...
		<div class="toBlock">
			...
			if (block1 > 1) {
				//[맨처음]
				out.print("&nbsp[<a href='testpaging.jsp?pageTo=1" +  "'>" + "맨처음</a>]&nbsp");

				//이전 블록 시작 페이지 = (현재)블럭 시작페이지 번호 - 블록당 페이지 수;
				previousBlockStartPageNo1 = blockStartPageNo1 - PAGE_PER_BLOCK;

				//[이전]
				out.print("&nbsp[<a href='testpaging.jsp?pageTo=" + previousBlockStartPageNo1  + "'>이전</a>]&nbsp");
			}
			...
			for (int i = blockStartPageNo1; i <= blockEndPageNo1; i++) {
				...
				if (i == curPageNo1)
					...
					out.print("&nbsp[<a href='testpaging.jsp?pageTo=" + i + "'>"  + i + "</a>]");
				// i가 총페이지를 넘어가면 반복 종료
				// i가 현재 페이지와 같으면 i출력
				// 그 외에는 [i]출력
			...
			if (blockEndPageNo1 < totalPage1) {
				...
				out.print("&nbsp[<a href = 'testpaging.jsp?pageTo=" + nextBlockStartPageNo1  + "'>다음</a>]&nbsp");
				out.print("&nbsp[<a href = 'testpaging.jsp?pageTo=" + totalPage1 + "'>맨끝</a>]&nbsp");
			}
			%>

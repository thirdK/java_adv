...
<body>
	...
	<%
		...
		if(request.getParameter("pageNo") == null){
			curPageNo = 1;
		}else if(totalPage < Integer.parseInt(request.getParameter("pageNo"))){
			curPageNo = totalPage;
		}else {
			curPageNo = Integer.parseInt(request.getParameter("pageNo"));
		}
	...
	if(block > 1){
		//[맨처음]
		out.print("&nbsp[<a href='notifyList.jsp?pageNo=1'>"+"맨처음</a>]&nbsp");
		
		//이전 블록 시작 페이지 = (현재)블럭 시작페이지 번호 - 블록당 페이지 수;
		previousBlockStartPageNo = blockStartPageNo - PAGE_PER_BLOCK;
		
		//[이전]
		out.print("&nbsp[<a href='notifyList.jsp?pageNo="
		+ previousBlockStartPageNo
		+"'>이전</a>]&nbsp");
	}
	...
	for(int i = blockStartPageNo; i <= blockEndPageNo; i++){
		...
		if(i == curPageNo) out.print("&nbsp" + i + "&nbsp");
		else out.print("&nbsp[<a href='notifyList.jsp?pageNo=" + i +"'>" + i + "</a>]");
		// i가 총페이지를 넘어가면 반복 종료
		// i가 현재 페이지와 같으면 i출력
		// 그 외에는 [i]출력
...
<!-- ==================================================================================  -->
	...
	<%-- <%@ include file="pagingPrepare.jsp" %> --%>
		<div class="fromList">
			<div class="notify_row">
				<div>내가 받은 목록</div>
			</div>

...
상황 1로 생각하면 k=2를 2개의 색이라고 생각하지만 
...
일단 n==1 이면 당연히 k를 반환한다. (전체 울타리 수가 1개라면 k개의 색으로 칠하는 경우의 수는 당연히 주어진 색(k)만큼이다.)
 
n==2 일때도 생각해야한다 울타리의 총합이 2개라면 경우의 수 k*k 이다.
(울타리 2개 까지는 연속되는 색에 대한 문제를 고민할 필요가 없으므로 k의 제곱이다.)

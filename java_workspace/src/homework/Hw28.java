...
k=2 이면 [-1/-2], [-1/-2] 를 할 수 있다. 색만 다를 뿐 어떠한 상황에서도 -1 아니면 -2이다.


 이제 재귀적으로 생각하면 된다. 
 모든식이 만들어 졌다고 가정한다. 마지막 결과 직전의 상황 울타리 1개만 남았을 때의 경우의 수를 생각한다.  
 모든 경우의 수는 n-1/n-2로 완성되었을 것이다
즉, 마지막 울타리 1개를 제외한 경우의 수는 solution(n-1) + solution(n-2) 으로 생각한다.
 ...
 문제는 이 다음인데 재귀함수를 2개를 써서 n==2일때 반환값도 생각해야한다.
 
 딱히 찾는 방법은 모르겠고 그냥 낮은 숫자로 노가다해서 규칙찾는게 최선인거 같다.

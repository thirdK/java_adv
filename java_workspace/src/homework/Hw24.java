...
2. 배열의 처음부터 마지막까지 반복문을 돌려서 탐색한다.
	-> 값이 0이면 dfs 탐색으로 인접한 0을 모두 1로 바꾸고 카운트한다. (방문한 배열을 1로 바꾸어 중복 탐색을 막는다.)

3. dfs를 이용하므로 stack / 재귀 2가지 방법 사용가능하다.

	Stack사용
	해당 인덱스의 값이 0이면 count를 +1하고 해당 인덱스를 stack에 push하고  pop하여 인접한 칸들의 값을 탐색한다.
	-> 노드가 아니라 2차원배열이므로 상하좌우 좌표로 이동해야 할거 같다.
	-> 아니면 4방향으로 link필드를 가지는 노드를 만들어야되는데... 하면 안될거 같다

	인접한 칸들의 값을 확인하여 0이면 stack에 push하고 모든 칸의 탐색이 끝나면  pop하여 다시 
	탐색 -> 0이면 push -> pop -> 탐색 ....을 반복한다. (pop할때마다 값을 1로 변경하자)

3-1. Stack으로 우선 풀고 재귀로 시도해보자

4. count를 한번만할지 계속카운트하여 끝나면 list에 추가하여 list.size()를 구할지는 구현을 해봐야할거 같다.
	->해보니까 stack은 카운팅이 가능하고 재귀는 힘들어서 size()를 이용하게됨

 */

public class Hw24 {
	static int[][] arr;
	static int cnt, count;	//Stack풀이는 cnt를 사용, 재귀 풀이는 count사용
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < arr.length; i++) {
			for (int j = 0; j < arr[i].length; j++) {
				if (arr[i][j] == 0) { // ***여기서 0을 발견하면 dfs()메소드가 실행됨***
	...
	static int dfsR(int x, int y) {	
		...
		count++;//count는 무의미하다 어차피 List의 길이로 답을 구할거라 아무 값이나 반환해도된다. 반환을 하는게 중요
